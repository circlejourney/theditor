...
$(window).on("load", function(){
    ...
    var tick = setInterval(function() {
        if(textChanged) {
            localStorage.th_cj_text_unstable = text_editor.getValue();
        ...
        if(htmlChanged) {
            localStorage.th_cj_unstable = editor.getValue();
            if(editor.getValue()) {
                $("#clear-html")
                    .html("<i class='fa fa-trash'></i>")
                    .attr("data-original-title", "Clear")
        ...
        if(cssChanged) {
            localStorage.th_cj_css_unstable = css_editor.getValue();
        ...
        if($("#edit-preview").prop("checked")) {
            if(frame.contentWindow.before && frame.contentWindow.before !== frame.contentWindow.$(".ace-code-container").html()) {
                frame.contentWindow.before = frame.contentWindow.$(".ace-code-container").html();
                editor.setValue(frame.contentWindow.$(".ace-code-container").html());
            } else if(!frame.contentWindow.before) {
                frame.contentWindow.before = frame.contentWindow.$(".ace-code-container").html();
            }
            
        }
        
        if($("#auto").prop("checked") || justLanded) {
            if(htmlChanged) {
                updateHTML();
                htmlChanged=false;
            }
...
function loadLocal() {
    if(localStorage.th_cj_unstable) {
        showval = localStorage.th_cj_unstable;
    }
    ...
    if(localStorage.th_cj_css_unstable) {
        showcss = localStorage.th_cj_css_unstable;
    }
    
    if(localStorage.th_cj_text_unstable) {
        showtext = localStorage.th_cj_text_unstable;
    }
    ...
    if(localStorage.th_cj_auto) {
        if(localStorage.th_cj_auto == "true") {
            $("#auto").prop("checked", "true");
        }
    }
    
    if(localStorage.th_cj_preview) {
        var savedPreview = localStorage.th_cj_preview;
        $("#"+savedPreview).prop("checked", "true");
    }
...
function updateCSS() {
    ...
    frame.contentWindow.updateCSS(raw_css);
}
...
function setAutoUpdate() {
    $("input[name=update]").each(function(){
        if($(this).prop("checked")) {
            localStorage.th_cj_preview = $(this).prop("id");
            
            if($(this).prop("id") == "auto") {
                frame.contentWindow.$(".ace-code-container").attr("contenteditable", "false");
                updateCode();
                
            } else if($(this).prop("id") == "edit-preview") {
                frame.contentWindow.$(".ace-code-container").attr("contenteditable", "true");
